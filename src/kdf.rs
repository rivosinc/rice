...
pub(crate) fn kdf<D: EagerHash>(
    ...
    let kdf = Hkdf::<D>::new(Some(salt), input_key_material);
...
pub(crate) fn derive_cdi_id<D: EagerHash>(public_key: &[u8], cdi_id: &mut [u8]) -> Result<()> {
    kdf::<D>(public_key, &ID_SALT, &[b"CDI_ID"], cdi_id)
}

/// Extract a CDI from antoher one.
/// This is mostly useful for expanding secrets into static lenghth CDIs.
pub fn extract_cdi<D: EagerHash>(cdi: &[u8], new_cdi: &mut [u8]) -> Result<()> {
    let kdf = Hkdf::<D>::new(None, cdi);
...
mod tests {
    ...
    fn test_kdf_invalid_okm() {
        ...
        assert!(kdf::<sha2::Sha384>(&ikm, &ID_SALT, &[], &mut okm).is_err());
    }
}
